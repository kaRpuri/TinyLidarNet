...
def read_ros2_bag(bag_path):
    ...
    while reader.has_next():
        ...
        elif topic == 'odom':  # Changed from 'Ackermann'
            msg = deserialize_message(data, Odometry)
            servo_data.append(msg.twist.twist.angular.z)  # Steering from angular.z
            speed_data.append(msg.twist.twist.linear.x)   # Speed from linear.x
...
if __name__ == "__main__":
    ...
    converter = tf.lite.TFLiteConverter.from_keras_model(model)
    # Tell TFLite it may keep TensorFlow ops that it can’t rewrite

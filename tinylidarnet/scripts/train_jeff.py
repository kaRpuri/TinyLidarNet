...
if __name__ == "__main__":
    # Configuration
    model_name = 'TLN_multi_input_safe'
    ...
    # Convert to arrays
    lidar = np.expand_dims(np.array(lidar), -1)
    test_lidar = np.expand_dims(np.array(test_lidar), -1)
    ...
    if max_speed - min_speed < epsilon:
        print("⚠️ Speed variation too small — flattening to zeros.")
    ...
    # Model Definition
    # ========================================================
    lidar_input = tf.keras.Input(shape=(lidar.shape[1], 1), name='lidar')
    x1 = tf.keras.layers.Conv1D(24, 10, strides=4, activation='relu')(lidar_input)
    x1 = tf.keras.layers.Conv1D(36, 8, strides=4, activation='relu')(x1)
    ...
    plt.savefig('./Figures/loss_curve_safe.png')

    converter = tf.lite.TFLiteConverter.from_keras_model(model)
    converter.target_spec.supported_ops = [tf.lite.OpsSet.TFLITE_BUILTINS, tf.lite.OpsSet.SELECT_TF_OPS]
    tflite_model = converter.convert()

    with open('./Models/TLN_Jeff_Safe.tflite', 'wb') as f:
